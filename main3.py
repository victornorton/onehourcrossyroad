...
class Chicken(Entity):
    ...
    def update(self):
        ...
        # 1. Check for Bounds (Drifting off screen on a log)
        if self.y < -5 or abs(self.x) > 12: 
            self.die('fall')
        ...
        # 4. SAFETY CHECK (The Raycast)
        # We perform this check only when NOT moving (idling on a tile)
        if not self.is_moving:
            # Cast ray from player center down
            hit_info = raycast(self.world_position + Vec3(0, 0.5, 0), Vec3(0, -1, 0), distance=2, ignore=(self,))
            
            if hit_info.hit:
                # HIT LOG: Safe. Attach player to log to ride it.
                if hit_info.entity.tag == 'log':
                    self.parent = hit_info.entity 
                
                # HIT WATER: Death. (Only happens if ray missed the log)
                elif hit_info.entity.tag == 'water':
                    self.parent = scene # Detach from any previous log
                    self.die('water')
                
                # HIT GROUND: Safe. Detach from log.
                else:
                    self.parent = scene
    ...
    def input(self, key):
        ...
        if dx != 0 or dz != 0:
            ...
            # IMPORTANT: We must calculate from world_position because self.position 
            # might be relative to a log right now.
            start_pos = self.world_position
            target_pos = start_pos + Vec3(dx, 0, dz)

            # Bounds Check
            if abs(target_pos.x) > 4: return 

            # Obstacle Check (Tree/Rock)
            hit = raycast(start_pos + Vec3(0,0.5,0), Vec3(dx, 0, dz), distance=1, ignore=(self,))
            if hit.hit and hit.entity.tag == 'obstacle':
                return 

            # EXECUTE MOVE
            self.is_moving = True
            self.parent = scene # DETACH immediately before jumping
            self.position = start_pos # Snap to world coordinates so we don't teleport
            
            if sfx_jump.clip: sfx_jump.play()
            
            # Animate to the integer grid target
            # We round target_pos to ensure we land exactly on the grid (0, 1, 2) 
            # rather than decimal offsets caused by floating point errors.
            clean_target = (round(target_pos.x), 1, round(target_pos.z))
            
            self.animate_position(clean_target, duration=0.1, curve=curve.linear)
            self.animate_y(1.5, duration=0.05, curve=curve.out_sine) # Jump arc up
            invoke(self.land, delay=0.1)

    def land(self):
        self.y = 1 # Snap exact height
        self.is_moving = False
    ...
    def die(self, cause):
        ...
        if not game_over:
            game_over = True
            self.parent = scene # Ensure we aren't attached to a moving object anymore
            
            if cause == 'water':
                if sfx_splash.clip: sfx_splash.play()
                self.animate_position((self.x, -1, self.z), duration=0.5) # Sink effect
            else:
...
class Log(Entity):
    def __init__(self, position, speed):
        super().__init__(
            ...
            # Y=0 makes it flush with the road/grass, floating above water
            position=position, 
            collider='box', tag='log'
        )
        self.speed = speed
...
class TerrainManager(Entity):
    ...
    def spawn_lane(self, z_pos):
        ...
        if lane_type == 'grass':
            ...
            # Water is LOWERED to -0.2 so the log (at 0.0) floats above it
